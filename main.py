...
class MemoryAllocationSimulator:
    def __init__(self, root):
        ...
        self.memory_size = 1024  # Total memory size in MB
    ...
    def setup_controls(self):
        ...
        self.size_label = ttk.Label(self.control_panel, text="Block Size (MB):")
    ...
    def update_canvas(self):
        ...
        for status, size in self.memory_blocks:
            ...
            self.ax.text(size / 2, y_pos, f'{status} ({size} MB)', va='center', ha='center', color='white')
            y_pos += 1
        self.ax.set_xlim(0, self.memory_size)
        self.ax.set_yticks([])
        self.ax.set_xticks(range(0, self.memory_size + 1, 100))
        self.ax.set_xlabel('Memory Size (MB)')
    ...
    def update_status(self):
        ...
        self.status.set(f'Total Memory: {self.memory_size} MB, Allocated: {total_allocated} MB, Free: {total_free} MB')
